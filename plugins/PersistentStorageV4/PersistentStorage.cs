...
namespace X13.PLC {
  ...
  public class PersistentStorage : IPlugModul {
    ...
    private const string DB_PATH = "../data/persist.xdb";
    ...
    public void Init() {
      ...
      _file = new FileStream(DB_PATH, FileMode.OpenOrCreate, FileAccess.ReadWrite);
      if(_file.Length < 0x40) {
        ...
        Load();
        _nextBak = DateTime.Now.AddDays(1);
      }
    ...
    private void Load() {
      ...
      // Refresh file & set values
      _now = DateTime.Now;
      string pathBak = "../data/" + LongToString(_now.Ticks * 3 / 2000000000L);  // 1/66(6)Sec.

      var tro = _tr;
      _tr = new Dictionary<Topic, Record>((tro.Count/512+2)*512);
      Queue<Topic> prq = new Queue<Topic>();
      prq.Enqueue(Topic.root);
      Topic ct;
      Record rv, rp;

      var file = new FileStream(pathBak + ".src", FileMode.Create, FileAccess.ReadWrite);
      file.Write(new byte[0x40], 0, 0x40);

      byte[] rBuf = new byte[64], dBuf = Record.dBuf;

      while(prq.Count>0) {
        ct = prq.Dequeue();
        if(tro.TryGetValue(ct, out rv)) {
          // save record
          curPos = file.Position;
          if(rBuf == null || rBuf.Length < ((rv.size + 15) & FL_LEN_MASK)) {
            rBuf = new byte[(rv.size + 15) & FL_LEN_MASK];
          }
          if(ct.parent != null) {
            if(!_tr.TryGetValue(ct.parent, out rp)) {
              Log.Warning("PersistentStorage.Refresh - no parent for: " + ct.path);
              continue;
            }
            rv.parent = rp.pos;
          } else {
          ...
          rv.pos = (uint)(curPos >> 4);
          CopyBytes(rv.fl_size, rBuf, 0);
          CopyBytes(rv.parent, rBuf, 4);
          Encoding.UTF8.GetBytes(rv.name).CopyTo(rBuf, 12);

          if(rv.data_size > 0) {
            //data
            byte[] tBuf = Encoding.UTF8.GetBytes(WOUM.ExConverter.Type2Name(rv.type));
            byte[] pBuf = Encoding.UTF8.GetBytes(rv.payload);
            rv.data_size = tBuf.Length + 1 + pBuf.Length;
            if(dBuf == null || dBuf.Length < ((rv.data_size + 6 + 15) & FL_LEN_MASK)) {
              dBuf = new byte[(rv.data_size + 6 + 15) & FL_LEN_MASK];
            }
            ...
            //record
            if(rv.saved_fl == FL_SAVED_I) {
              CopyBytes(rv.data_size, rBuf, 8);
              Buffer.BlockCopy(dBuf, 4, rBuf, rv.size - rv.data_size - 2, rv.data_size);
            } else {
              CopyBytes(6 + rv.data_size, dBuf, 0);
              rv.data_pos = (uint)(rv.pos + ((rv.size + 15)>>4));
              CopyBytes(rv.data_pos, rBuf, 8);
            }
          } else {
            CopyBytes(rv.data_size, rBuf, 8);
          }
          WriteRef(file, rv.pos, rBuf, rv.size);
          if(rv.saved_fl == FL_SAVED_E) {
            WriteRef(file, rv.data_pos, dBuf, rv.data_size + 6);
          }

          _tr[ct] = rv;
          //
          if(rv.saved && !string.IsNullOrEmpty(rv.payload)) {
            ct.FromJson(rv.payload, _sign);
          }
          foreach(var ch in ct.children){
            if(ch.path != "/local") {
              prq.Enqueue(ch);
            }
          }
      ...
      File.Replace(pathBak + ".src", DB_PATH, pathBak + "$.bak", true);
      _file = new FileStream(DB_PATH, FileMode.OpenOrCreate, FileAccess.ReadWrite);
    }
    ...
    private void WriteRef(FileStream file, uint pos, byte[] buf, int len) {
