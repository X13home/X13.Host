...
function minicolorsWidget() {
  ...
  return {
        ...
        $(this).minicolors({
          ...
          change: function (hex, opacity) {
            ...
            if ((c = parseInt(hex.substring(1), 16)) != null) {
              pubsub.publish($(el).data("pub"), c);
              console.log("color=" + hex);
            }
          },
        });
        $.getJSON($(el).data("pub"), function (v) {
          $(el).minicolors('value', pack(v));
        });
      });
    ...
    sub: function (w) {
      w.find(".colorpicker[data-sub]").each(function (idx, el) {
        pubsub.subscribe($(el).data("sub"));
      });
    },
    pub: function (w, topic, v) {
      w.find(".colorpicker[data-sub='" + topic + "']").each(function (idx, el) {
        $(el).minicolors('value', pack(v));
      });
    }
  };
