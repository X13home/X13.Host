...
namespace Jurassic.Library {
  ...
  internal sealed class JSONSerializer {
    ...
    private void SerializePropertyValue(object value, StringBuilder result)
        {
            ...
            {
                if ((bool)value == false)
                    result.Append(BooleanConstructor.FalseString);
                else
                    result.Append(BooleanConstructor.TrueString);
                return;
            }
          ...
          if(value is DateTime) {
            result.Append(((DateTime)value).ToUniversalTime().ToString("yyyy-MM-dd'T'HH:mm:ss.fff'Z'", System.Globalization.DateTimeFormatInfo.InvariantInfo));
            return;
          }
    ...
    internal static void QuoteString(string input, System.Text.StringBuilder result) {
      result.Append('\"');
