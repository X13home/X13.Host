...
#pragma warning restore 0618
    ...
    [Export(typeof(IStatement))]
    [ExportMetadata("declarer", "Fade")]
    private class Fade : IStatement {
      private DVar<long> _set, _out, _t;
      private Timer _ct;
      private double _delta;

      public void Load() {
        var m=Topic.root.Get<string>("/etc/declarers/func/Fade");
        m.value="pack://application:,,/CC;component/Images/bi_fade.png";
        m.Get<string>("_description").value="d Fade";
        m.Get<string>("Set").value="Ai";
        m.Get<string>("Out").value="ai";
        m.Get<string>("rename").value="|R";
        m.Get<string>("remove").value="}D";
      }

      public void Init(DVar<PiStatement> model) {
        _set=AddPin<long>(model, "Set");
        _out=AddPin<long>(model, "Out");
        _out.saved=false;
        bool t_p=model.Exist("_t");
        _t=AddPin<long>(model, "_t");
        ...
        _ct=new Timer(Pool, null, 5000, 100);
      }

      public void Calculate(DVar<PiStatement> model, Topic source) {
        if(source!=_out) {
          if(_set.value!=_out.value) {
            double tmp=(_out.value>0?Math.Sqrt(_out.value):-Math.Sqrt(-_out.value));
            _delta=(((_set.value>0?Math.Sqrt(_set.value):-Math.Sqrt(-_set.value))-tmp)/(_t.value/100.0));
            _ct.Change(20, 100);
          }
        }
      }
      ...
      private void Pool(object o) {
        ...
        double tmp=(_out.value>0?Math.Sqrt(_out.value):-Math.Sqrt(-_out.value));
        long rez=(long)((tmp+_delta)*(tmp+_delta>=0?tmp+_delta:-(tmp+_delta)));
        if(_set.value>_out.value) {
          if(rez>_set.value) {
            _out.value=_set.value;
          } else if(rez<=_out.value) {
            _out.value+=1;
          } else {
            _out.value=rez;
          }
